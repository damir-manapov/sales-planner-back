...
describe('BrandsService', () => {
  ...
  beforeEach(() => {
    ...
    service = new BrandsService(mockDb as DatabaseService);
  });
  ...
  describe('findByShopId', () => {
    it('should call database with correct shop_id', async () => {
      ...
      expect(mockDb.selectFrom).toHaveBeenCalledWith('brands');
  ...
  describe('exportForShop', () => {
    it('should return brands with only code and title', async () => {
      const mockBrands = [
        { code: 'mavyko', title: 'Мавико' },
        { code: 'marshall', title: 'MARSHALL' },
      ];

      const mockExecute = vi.fn().mockResolvedValue(mockBrands);
