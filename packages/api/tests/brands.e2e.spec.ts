...
describe('Brands (e2e)', () => {
  ...
  describe('CRUD operations', () => {
    it('should create brand', async () => {
      ...
      expect(brand.shopId).toBe(ctx.shop.id);
      expect(brand.tenantId).toBe(ctx.tenant.id);
    });
  ...
  describe('Tenant-based access control', () => {
    ...
    it('should allow same code in different tenants', async () => {
      ...
      expect(brand1.tenantId).not.toBe(brand2.tenantId);
    });
  });
  ...
  describe('Import/Export', () => {
    ...
    it('should reject import with duplicate codes', async () => {
      const code = generateTestCode('dup-brand');
      const csvContent = `code,title\n${code},First Brand\n${code},Second Brand`;

      try {
        await ctx.client.brands.importCsv(ctx.shopContext, csvContent);
